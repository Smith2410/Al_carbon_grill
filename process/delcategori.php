...
$idCateg=consultasSQL::clean_string($_POST['categ-id']);
$cons=ejecutarSQL::consultar("SELECT * FROM producto WHERE id='$idCateg'");
if(mysqli_num_rows($cons)<=0){
    if(consultasSQL::DeleteSQL('categoria', "id='".$idCateg."'")){
        echo '<script>
		    swal({
