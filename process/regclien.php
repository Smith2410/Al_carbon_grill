...
$userClient=consultasSQL::clean_string($_POST['clien-user']);
...
if(!$dniClient=="" && !$userClient=="" && !$apeClient=="" && !$dirClient=="" && !$telClient=="" && !$nomClient==""){
    if($passClient==$passClient2){
        $verificar= ejecutarSQL::consultar("SELECT * FROM cliente WHERE DNI='".$dniClient."'");
        $verificaltotal = mysqli_num_rows($verificar);
        if($verificaltotal<=0){
            if(consultasSQL::InsertSQL("cliente", "DNI, Usuario, Nombre, Apellidos, Direccion, Clave, Telefono", "'$dniClient','$userClient','$nomClient','$apeClient','$dirClient', '$passClient','$telClient'")){
                echo '<script>
                    swal({
